...
func TestProtoSerializer_Marshal(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "test-1",
			arg:  &message.ArithRequest{A: 1, B: 2},
			expect: expect{
...
func TestProtoSerializer_Unmarshal(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "test-1",
			arg:     []byte{0x9, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x11, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
			message: &message.ArithRequest{},
			expect: expect{
				message: &message.ArithRequest{A: 1, B: 2},
				err:     nil,
			},
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if err != nil {
				assert.Equal(t, c.expect.message.(*message.ArithRequest).A,
					c.message.(*message.ArithRequest).A)
				assert.Equal(t, c.expect.message.(*message.ArithRequest).B,
					c.message.(*message.ArithRequest).B)
			}
